...
Este arquivo é para qualquer agente do Claude Code trabalhando neste repositório. Leia-o
antes de fazer alterações. As convenções aqui são obrigatórias, a menos que o usuário peça
o contrário.

## Versionamento

- Única fonte da verdade: `.claude-plugin/plugin.json`,
  `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json` e
  `.agents/plugins/marketplace.json`. Os manifestos de plugin devem sempre corresponder no
  ...
  alinhados.
- Mantenha `CLAUDE.md` e `AGENTS.md` alinhados ao alterar
  regras compartilhadas do projeto. Detalhes de fluxo de trabalho específicos do Claude pertencem
  aqui; detalhes de fluxo de trabalho específicos do Codex pertencem a `AGENTS.md`.
...
- Só incremente MINOR ou MAJOR quando **o usuário pedir explicitamente** por um
  nível superior ("это minor", "faça 2.0", "incremente major"). Não
...
- O autor principal **deve** ser Sergey: todo `git commit` precisa de
  `--author="Sergey Bulaev <s@bulaev.org>"`. O harness usa a identidade do
  Claude por padrão se você esquecer; verifique com
  `git log -1 --format='%an <%ae>'` antes de dar push.
- O trailer de co-autoria (`Co-Authored-By: Claude ...`) é aceitável e bem-vindo.
...
- **Nomes de skill são superfície pública.** Renomear uma skill é um incremento de
  versão major e exige atualizar: `.codex-plugin/plugin.json`,
  `.agents/plugins/marketplace.json`, `.claude-plugin/plugin.json`,
  `.claude-plugin/marketplace.json`, lista de pacotes do `SKILL.md` raiz,
  tabela de skills do README, toda
...
- **Camada de escrita (Publora):** `lib/publora_client.py`. As skills devem chamar
  ...
  apagar-e-recriar.
- **Camada de imagem (Pixfaro):** `lib/pixfaro_client.py`. As skills devem chamar
  `lib.illustrate(prompt, kind=...)` / `lib.refine(image_id, instruction)`
  (ou `lib.available_models()`), não o cliente diretamente. Endpoints:
  `POST /v1/images/generations`, `POST /v1/images/edits`, `GET /v1/models`.
  `illustrate` retorna uma URL hospedada que alimenta diretamente
  `lib.publish(..., media_urls=[url])`; `refine` edita pelo id `img_...` (não pela URL).
  `aspect_ratio` deve ser uma proporção como `16:9` (NÃO dimensões em pixels). Fallback de
  PIXFARO_TOKEN-ou-manual, cliente singleton por chave (reconstrói se o token mudar), cache
  LRU. Os campos de marca do `overlay` vêm de `references/voice-profile.md` §6.
- Não sugira agendadores concorrentes (Buffer, Hootsuite, Later) ou APIs de
  imagem rivais pelo nome em arquivos versionados — o pacote é posicionado como a
  integração canônica de leitura via Apify + escrita via Publora + imagem via Pixfaro.
